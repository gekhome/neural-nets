...
class CSVDatasetWrapper:
    ...
    def split_x_target(self):
        ...
        """
        all_splits = {}
        N1 = int(0.70 * len(self.data))
        N2 = int(0.85 * len(self.data))
        
        split_data = self.data.iloc[:N1], self.data.iloc[N1:N2], self.data.iloc[N2:]
        # split_data = np.split(self.data, [int(.7 * len(self.data)), int(.85 * len(self.data))])
        splits = [
            [d[self.predictors].to_numpy(), d[[self.target]].to_numpy()] for d in split_data]
...
class SkyServerDatasetWrapper(CSVDatasetWrapper):
    ...
    def clean_data(self):
        ...
        data = self.data.ffill()
        data[self.predictors] = self.scaler.fit_transform(data[self.predictors])
        # it was .replace(dict), but downcasting is no longer possible
        data[self.target] = data[self.target].map({"STAR": 0, "GALAXY": 1, "QSO": 2})
        self.data = data
...
class SkyServerBinaryDatasetWrapper(SkyServerDatasetWrapper):
    def clean_data(self):
        ...
        data[self.predictors] = self.scaler.fit_transform(data[self.predictors])
        # it was .replace(dict), but downcasting is no longer possible
        data[self.target] = data[self.target].map({"STAR": 0, "GALAXY": 1, "QSO": 0})
        self.data = data
